...
class VLTAPolicy(nn.Module):
    def __init__(
        ...
        deform_vit = TinyViT_TactileImageEncoder(
            image_size=tactile_image_size, patch_size=tactile_patch_size, n_hands=n_hands, n_fingers=n_fingers
        )
        raw_vit = TinyViT_TactileImageEncoder(
            image_size=tactile_image_size, patch_size=tactile_patch_size, n_hands=n_hands, n_fingers=n_fingers
        )
        tactile_image_encoder = TactileImageEncoder(deform_encoder=deform_vit, raw_encoder=raw_vit)
    ...
    #     return self.action_head.compute_loss(obs_tokens, target_action, action_is_pad=action_is_pad)
    def compute_loss(
            self,
            obs: Observation,
            target_action: torch.Tensor,
            ...
            obs_tokens = self.encode_observation(obs)
            return self.action_head.compute_loss(obs_tokens, target_action, loss_mask=action_is_pad)

    @torch.no_grad()
    def sample_actions(self, obs: Observation) -> torch.Tensor:
        obs_tokens = self.encode_observation(obs)
        return self.action_head.sample_actions(obs_tokens).reshape(obs.batch_size, self.action_head.chunk_size, self.action_head.dim_action)    
    
    def forward(self, obs: Observation) -> torch.Tensor:
        return self.sample_actions(obs).reshape(obs.batch_size, self.action_head.chunk_size, self.action_head.dim_action)